...
UPDATE user u SET total_chat = (SELECT COUNT(*) FROM chat c WHERE c.status in (1,2) AND c.owner_id = u.id and c.location_id = u.location_id);
UPDATE user u SET latest_chat = (SELECT title FROM chat c WHERE c.status in (1,2) AND c.owner_id = u.id and c.location_id = u.location_id order by c.id limit 1);
